...
export default class ProcessedQuery {
	...
	putAll(keyValuePairs: { [key: string]: any }): ProcessedQuery {
		Object.entries(keyValuePairs).forEach(([key, value]: [string, any]) => this.put(key, value));
		return this;
	}
	...
	putObject(key: string, value: object): void {
		Object.entries(value).forEach(([entry, entryValue]: [string, string | number]) => {
	...
	stringify(): string {
		return `?${querystring.stringify(this.processedQuery as ParsedUrlQueryInput)}`;
	}
}
